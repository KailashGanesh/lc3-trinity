...
def run_test_suite():
    os.system("")  # This line helps enable ANSI color support on some Windows terminals
    ...
    for file_name in test_files:
        with open(os.path.join(test_dir, file_name), "r") as f:
            ...
            print(
                f"{Colors.HEADER}--- Running tests for {instruction} ---{Colors.ENDC}"
            )
            ...
            for test in data["tests"]:
                ...
                if "registers" not in test["final_state"]:
                    print(
                        f"{Colors.FAIL}Test not valid! no registers expected in final_state{Colors.ENDC}"
                    )
                ...
                for reg, expected_val in test["final_state"]["registers"].items():
                    ...
                    if actual_val != expected_val:
                        ...
                        print(
                            f"    - Register {Colors.BOLD}{reg}{Colors.ENDC}: expected {hex(expected_val)}, got {hex(actual_val)}"
                        )
                        test_passed = False
    ...
    print(
        f"Passed: {passed_color}{total_passed}{Colors.ENDC}, Failed: {Colors.FAIL}{total_failed}{Colors.ENDC}"
    )
